...
public record CamouflageTexture {
	public CamouflageTexture(XElement texture) {
		...
		if (texture.Element("Influence_ao") is not { } xInfluenceAO ||
			!float.TryParse(xInfluenceAO.Value, NumberStyles.Float, CultureInfo.InvariantCulture,
				out var influenceAO)) {
			influenceAO = 1f;
		}
		...
		if (texture.Element("camoEmissionBasePower") is { } xCamoEmissionBasePower &&
			float.TryParse(xCamoEmissionBasePower.Value, NumberStyles.Float, CultureInfo.InvariantCulture,
				out var camoEmissionBasePower)) {
		...
		if (texture.Element("camoEmissionAnimationMaxPower") is { } xCamoEmissionAnimationMaxPower &&
			float.TryParse(xCamoEmissionAnimationMaxPower.Value, NumberStyles.Float, CultureInfo.InvariantCulture,
				out var camoEmissionAnimationMaxPower)) {
			CamoEmissionAnimationMaxPower = camoEmissionAnimationMaxPower;
		}

		if (texture.Element("camoMaskSmooth") is { } xCamoMaskSmooth &&
			float.TryParse(xCamoMaskSmooth.Value, NumberStyles.Float, CultureInfo.InvariantCulture,
				out var camoMaskSmooth)) {
		...
		if (texture.Element("camoMaskColor2Alpha") is not { } xCamoMaskColor2Alpha ||
			!float.TryParse(xCamoMaskColor2Alpha.Value, NumberStyles.Float, CultureInfo.InvariantCulture,
				out var camoMaskColor2Alpha)) {
