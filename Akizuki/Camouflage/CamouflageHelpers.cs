...
public record CamouflageHelpers {
	public static Vector2D<float> ConvertVec2(string value) {
		...
		if (values.Length < 1 ||
			!float.TryParse(values[0], NumberStyles.Float, CultureInfo.InvariantCulture, out var u)) {
			u = 1.0f;
		}

		if (values.Length < 2 ||
			!float.TryParse(values[1], NumberStyles.Float, CultureInfo.InvariantCulture, out var v)) {
			v = 1.0f;
		}
	...
	public static Vector4D<float> ConvertVec4(string? value) {
		...
		if (values.Length < 4 ||
			!float.TryParse(values[3], NumberStyles.Float, CultureInfo.InvariantCulture, out var a)) {
			a = 1.0f;
		}
