...
// Original Port at
// https://github.com/ValveResourceFormat/ValveResourceFormat/blob/e11fb0742c948065fefac5cc7a95a9a7eedb900b/ValveResourceFormat/Compression/MeshOptimizerIndexDecoder.cs
...
/// </summary>
/// <seealso href="https://github.com/zeux/meshoptimizer/blob/master/src/indexcodec.cpp">
///     This is a C# port of
///     meshoptimizer.
/// </seealso>
...
public static class MeshOptimizerIndexDecoder {
	...
	public static byte[] DecodeIndexBuffer(int indexCount, int indexSize, ReadOnlySpan<byte> buffer) {
		...
		for (var i = 0; i < indexCount; i += 3) {
			...
			switch (codetri) {
				...
				default: {
					...
					PushVertexFifo(vertexFifo, ref vertexFifoOffset, a);
					PushVertexFifo(vertexFifo, ref vertexFifoOffset, b, feb is 0 or 15);
					PushVertexFifo(vertexFifo, ref vertexFifoOffset, c, fec is 0 or 15);
		...
		return position != data.Length ? throw new InvalidDataException("we didn't read all data bytes and stopped before the boundary between data and codeaux table") : destinationArray;
	}
}
