// SPDX-FileCopyrightText: 2019-2025 ValveResourceFormat, 2025 Legiayayana
//
// SPDX-License-Identifier: MIT
//
// C# Port of https://github.com/zeux/meshoptimizer/blob/master/src/indexcodec.cpp
// Original Port at
// https://github.com/ValveResourceFormat/ValveResourceFormat/blob/fd9bc105378040d9e0726f45dbb3b1d657d383de/ValveResourceFormat/Compression/MeshOptimizerVertexDecoder.Simd.cs
// Edited to use Spans instead of returning a byte[]
...
public static partial class MeshOptimizerVertexDecoder {
	...
	private static Span<byte> DecodeBytesSimd(Span<byte> data, Span<byte> destination) {
		...
		for (; i + ByteGroupSize * 4 <= destination.Length && data.Length >= ByteGroupDecodeLimit * 4; i += ByteGroupSize * 4) {
			...
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 0)..], (headerByte >> 0) & 3);
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 1)..], (headerByte >> 2) & 3);
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 2)..], (headerByte >> 4) & 3);
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 3)..], (headerByte >> 6) & 3);
		}
