...
// Original Port at
// https://github.com/ValveResourceFormat/ValveResourceFormat/blob/e11fb0742c948065fefac5cc7a95a9a7eedb900b/ValveResourceFormat/Compression/MeshOptimizerVertexDecoder.Simd.cs
...
public static partial class MeshOptimizerVertexDecoder {
	...
	private static Vector128<byte> DecodeShuffleMask(byte mask0, byte mask1) {
		...
		var off = Vector128.Create(DecodeBytesGroupCount[mask0]).AsInt64();
		var right = sm1 + off;

		return Sse2.UnpackLow(sm0, right).AsByte();
	}
	...
	private static ReadOnlySpan<byte> DecodeBytesGroupSimd(ReadOnlySpan<byte> data, Span<byte> destination, int hbits) {
		switch (hbits) {
			...
			case 6: {
				...
				var rest = Vector128.Create(data[4..]);
			...
			case 7: {
				...
				var sel4 = Vector64.Create(data[..8]).ToVector128();
				var rest = Vector128.Create(data[8..]);
			...
			case 5: {
				var mask0 = data[0];
				var mask1 = data[1];
				var rest = Vector128.Create(data[2..]);
	...
	private static void DecodeDeltas4Simd(int channel, ReadOnlySpan<byte> buffer, Span<byte> transposed, int vertexCountAligned, int vertexSize, ReadOnlySpan<byte> lastVertex, int rot) {
		...
		for (var j = 0; j < vertexCountAligned; j += 16) {
			var r0 = Vector128.Create(buffer[j..]);
			var r1 = Vector128.Create(buffer[(j + 1 * vertexCountAligned)..]);
			var r2 = Vector128.Create(buffer[(j + 2 * vertexCountAligned)..]);
			var r3 = Vector128.Create(buffer[(j + 3 * vertexCountAligned)..]);
		...
		try {
			...
			for (var k = 0; k < vertexSize; k += 4) {
				...
				for (var j = 0; j < 4; ++j) {
					...
					switch (ctrl) {
						// Literal encoding
						case 3 when data.Length < vertexCountAligned:
	...
	private static ReadOnlySpan<byte> DecodeBytesSimd(ReadOnlySpan<byte> data, Span<byte> destination, int hshift) {
		...
		for (; i + ByteGroupSize * 4 <= destination.Length && data.Length >= ByteGroupDecodeLimit * 4; i += ByteGroupSize * 4) {
			...
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 0)..], hshift + ((headerByte >> 0) & 3));
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 1)..], hshift + ((headerByte >> 2) & 3));
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 2)..], hshift + ((headerByte >> 4) & 3));
			data = DecodeBytesGroupSimd(data, destination[(i + ByteGroupSize * 3)..], hshift + ((headerByte >> 6) & 3));
		}
