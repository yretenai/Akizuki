// SPDX-FileCopyrightText: 2019-2026 ValveResourceFormat, 2026 Neptuwunium
//
// SPDX-License-Identifier: MIT
//
// C# Port of https://github.com/zeux/meshoptimizer/blob/master/src/vertexcodec.cpp
// Original Port at
// https://github.com/ValveResourceFormat/ValveResourceFormat/blob/e11fb0742c948065fefac5cc7a95a9a7eedb900b/ValveResourceFormat/Compression/MeshOptimizerVertexDecoder.cs
// edits: code quality and modern c# syntax
...
namespace Akizuki.Graphics.MeshCompression;

/// <summary>
///     Provides decoding functionality for mesh optimizer vertex buffers.
/// </summary>
/// <seealso href="https://github.com/zeux/meshoptimizer/blob/master/src/vertexcodec.cpp">
///     This is a C# port of
///     meshoptimizer.
/// </seealso>
...
public static partial class MeshOptimizerVertexDecoder {
	...
	private static uint Rotate32(uint v, int r) => (v << r) | (v >> ((32 - r) & 31));

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static byte Unzigzag8(byte v) => (byte) ((0 - (v & 1)) ^ (v >> 1));

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static ushort Unzigzag16(ushort v) => (ushort) ((0 - (v & 1)) ^ (v >> 1));
	...
	private static ReadOnlySpan<byte> DecodeBytesGroup(ReadOnlySpan<byte> data, Span<byte> destination, int bits) {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		byte Next(byte readBits, byte encv) {
			enc = b;
			enc >>= 8 - readBits;
			b <<= readBits;
	...
	private static void DecodeDeltas1(int size, ReadOnlySpan<byte> buffer, Span<byte> transposed, int vertexCount, int vertexSize, ReadOnlySpan<byte> lastVertex, int rot) {
	...
	private static ReadOnlySpan<byte> DecodeVertexBlock(ReadOnlySpan<byte> data, Span<byte> vertexData, int vertexCount, int vertexSize, Span<byte> lastVertex, ReadOnlySpan<byte> channels, int version) {
		if (vertexCount is <= 0 or > VertexBlockMaxSize) {
		...
		try {
			...
			for (var k = 0; k < vertexSize; k += 4) {
				...
				for (var j = 0; j < 4; ++j) {
					...
					switch (ctrl) {
						// Literal encoding
						case 3 when data.Length < vertexCount:
	...
	public static byte[] DecodeVertexBuffer(int vertexCount, int vertexSize, ReadOnlySpan<byte> buffer, bool useSimd = true) {
		...
		try {
			...
			var channels = version == 0 ? default : buffer.Slice(buffer.Length - tailSize + vertexSize, vertexSize / 4);
			...
			while (vertexOffset < vertexCount) {
				...
				buffer = useSimd ? DecodeVertexBlockSimd(buffer, vertexData, blockSize, vertexSize, lastVertex, channels, version) : DecodeVertexBlock(buffer, vertexData, blockSize, vertexSize, lastVertex, channels, version);
		...
		return buffer.Length != tailSizePadded ? throw new ArgumentException("Tail size incorrect") : resultArray;
	}
}
